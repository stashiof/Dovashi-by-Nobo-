        ...
        if (isCallActiveRef.current) {
          session.sendClientContent({
            turns: [{
              role: 'user',
              parts: [{
                text: `[SYSTEM: Start the call for Level ${pId}. Topic: "${struct}" (Meaning: ${meaning}). Speak ENTIRELY in Bengali! Act as the strict, sarcastic coach. Do NOT overact. Greet them shortly, then IMMEDIATELY give them a Bengali sentence related to "${struct}" and aggressively ask them to translate it to English! Do NOT tell any stories.]`
              }]
            }],
            turnComplete: true
          });
