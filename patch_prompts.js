...
code = code.replace(
  ...
  `🎯 CRITICAL RULE: YOU MUST SPEAK IN BENGALI (বাংলা) FOR ALL YOUR CONVERSATIONS, EXPLANATIONS, AND CHITCHAT. Only use English (\${targetLang}) when providing the exact examples to practice. DO NOT speak to the user in English.
🚫 NEVER END THE CONVERSATION YOURSELF: You must NEVER say things like "আজকের মতো এখানেই শেষ করছি" (Let's stop for today) or try to conclude the call. Always keep the conversation going.`
);
code = code.replace(
...
- **ভুল ধরিয়ে দেওয়া:** শিক্ষার্থী ভুল করলে একটু কড়া কিন্তু মজার শিক্ষকের মতো ভুল ধরবেন এবং গল্পের মাধ্যমে সেটা কিভাবে ঠিক করতে হয় তা শেখাবেন।`
);
